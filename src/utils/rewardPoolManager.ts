...
export function calculateRecommendedTopUp(
  ...
  // Average APY across all lock periods (weighted)
  const avgAPY = (
    STAKING_CONFIG.APY_RATES.flexible * 0.2 +
    STAKING_CONFIG.APY_RATES["30"] * 0.3 +
    STAKING_CONFIG.APY_RATES["90"] * 0.5
  );
...
export async function checkRewardPoolHealth(
  ...
  const recommendedTopUp = calculateRecommendedTopUp(totalValueLocked, minDaysCovered);
