...
export default function ContractManagementPanel({ isVisible, onClose }: ContractManagementPanelProps) {
      ...
      const list: ContractConfig[] = snap.docs.map(d => {
        ...
        if (Array.isArray(v.tiers)) {
          tiers = v.tiers.map((t: any) => ({
            minNFTs: Number(t.minNFTs || 0),
            maxNFTs: Number(t.maxNFTs || 0),
        ...
        return {
          ...
          tiers: tiers.length > 0 ? tiers : [{ minNFTs: 1, maxNFTs: 999, weeklyRate: 200 }],
  ...
  const handleDeactivate = async (contractAddress: string) => {
    ...
    setError(null);
    try {
      const contract = contracts.find(c => c.contract_address === contractAddress);
      if (!contract) return;

      const ref = doc(db, 'contractBonusConfigs', contractAddress);
      await updateDoc(ref, { is_active: !contract.is_active, updatedAt: serverTimestamp() });
      await fetchContracts();
    } catch (err) {
      ...
      <div className="bg-gradient-to-b from-[#1a1a1a] to-[#0f0f0f] border border-[#DA9C2F]/30 rounded-2xl max-w-6xl w-full max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-gradient-to-b from-[#1a1a1a] to-[#1a1a1a]/95 backdrop-blur-sm border-b border-[#DA9C2F]/20 p-6 z-10">
          <div className="flex justify-between items-center">
            <div>
              <h2 className="text-2xl font-bold text-[#DA9C2F]">Contract Reward Manager</h2>
              <p className="text-white/60 text-sm mt-1">Configure NFT collections and tier-based rewards</p>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="p-6">
          ...
          {showAddForm && (
            <form onSubmit={handleSubmit} className="mb-8 p-6 bg-[#0f0f0f]/50 rounded-lg border border-[#DA9C2F]/20">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  <label className="block text-white/80 text-sm mb-2">Contract Address *</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-white/80 text-sm mb-2">Magic Eden Symbol</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-white/80 text-sm mb-2">Collection Name *</label>
                  <input
                    type="text"
                    value={formData.name}
                ...
                <div>
                  <label className="block text-white/80 text-sm mb-2">Blockchain</label>
                  <select
                ...
                <div>
                  <label className="block text-white/80 text-sm mb-2">Welcome Bonus (MKIN)</label>
                  <input
                    type="number"
                    value={formData.welcomeBonus}
                    onChange={(e) => setFormData({ ...formData, welcomeBonus: Number(e.target.value) })}
                    className="w-full px-4 py-2 bg-[#1a1a1a] border border-[#DA9C2F]/30 rounded-lg text-white focus:outline-none focus:border-[#DA9C2F]"
                    min="0"
                  />
                </div>
              ...
              <div className="mb-4">
                <div className="flex justify-between items-center mb-3">
                  <label className="block text-white/80 text-sm font-semibold">Reward Tiers</label>
                  <button
                    type="button"
                    onClick={addTier}
                ...
                <div className="space-y-3">
                  {formData.tiers.map((tier, index) => (
                    <div key={index} className="grid grid-cols-[1fr_1fr_1fr_auto] gap-3 items-end p-3 bg-[#1a1a1a]/50 rounded-lg border border-[#DA9C2F]/10">
                      <div>
                        <label className="block text-white/60 text-xs mb-1">Min NFTs</label>
                        <input
                          type="number"
                          value={tier.minNFTs}
                          onChange={(e) => updateTier(index, 'minNFTs', Number(e.target.value))}
                      ...
                      <div>
                        <label className="block text-white/60 text-xs mb-1">Max NFTs</label>
                        <input
                          type="number"
                          value={tier.maxNFTs}
                          onChange={(e) => updateTier(index, 'maxNFTs', Number(e.target.value))}
                          className="w-full px-3 py-2 bg-[#0f0f0f] border border-[#DA9C2F]/30 rounded text-white text-sm focus:outline-none focus:border-[#DA9C2F]"
                          min={tier.minNFTs}
                        />
                      </div>
                      <div>
                        <label className="block text-white/60 text-xs mb-1">Weekly MKIN</label>
                        <input
                          type="number"
                          value={tier.weeklyRate}
                          onChange={(e) => updateTier(index, 'weeklyRate', Number(e.target.value))}
                          className="w-full px-3 py-2 bg-[#0f0f0f] border border-[#DA9C2F]/30 rounded text-white text-sm focus:outline-none focus:border-[#DA9C2F]"
                          min="0"
                        />
                      </div>
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-white mb-4">Existing Contracts</h3>
            {loading && !showAddForm && (
              <div className="text-center py-8 text-white/60">Loading contracts...</div>
            )}

            {!loading && contracts.length === 0 && (
              <div className="text-center py-8 text-white/60">
                <p>No contracts configured yet.</p>
                <p className="text-sm mt-2">Add your first contract to start earning rewards!</p>
              </div>
            )}
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  <div className="flex-1">
                    ...
                    <p className="text-white/60 text-sm font-mono mb-1">{contract.contract_address}</p>
                    {contract.magic_eden_symbol && (
                      <p className="text-white/40 text-xs">Magic Eden: {contract.magic_eden_symbol}</p>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-[#DA9C2F] font-semibold">{contract.welcome_bonus} MKIN</p>
                  </div>
                  <div>
                    <p className="text-white/60 text-xs mb-2">Blockchain</p>
                    <p className="text-white capitalize">{contract.blockchain}</p>
                  </div>
                </div>
                ...
                <div className="mt-4">
                  ...
                  <div className="space-y-2">
                    {contract.tiers.map((tier, idx) => (
                      <div key={idx} className="flex justify-between items-center p-2 bg-[#1a1a1a]/50 rounded border border-[#DA9C2F]/10">
