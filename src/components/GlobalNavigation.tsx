...
export default function GlobalNavigation() {
  ...
  const mainNavItems = useMemo(
    () => NAV_ITEMS.filter((item) => !item.group),
    []
  );
  const ecosystemItems = useMemo(
    () => NAV_ITEMS.filter((item) => item.group === "Ecosystem"),
    []
  );
  ...
  const formattedBalance = useMemo(
    () =>
      userRewards
        ? `${rewardsService.formatMKIN(userRewards.totalRealmkin)} MKIN`
        : "0.00 MKIN",
    [userRewards]
  );
      ...
      <nav className="hidden lg:flex fixed top-0 left-0 right-0 z-40 items-center justify-between px-8 py-4 bg-gradient-to-b from-[#050302]/95 to-[#050302]/80 backdrop-blur-md border-b border-[#DA9C2F]/10">
        ...
        <Link href="/" className="flex items-center gap-3">
          <Image
            ...
            height={40}
          />
          <span className="font-bold text-[#DA9C2F] uppercase tracking-wider">
            Realmkin
          </span>
        </Link>
        ...
        {/* Dynamic Island - Desktop Center Positioning */}
        <div className="absolute left-1/2 -translate-x-1/2 top-[72px] hidden lg:block pointer-events-none">
