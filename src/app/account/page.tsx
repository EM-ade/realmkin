...
export default function AccountPage() {
  ...
  const [revenueEligibility, setRevenueEligibility] =
    useState<RevenueEligibility | null>(null);
  const [revenueLoading, setRevenueLoading] = useState(false);
  const [revenueClaiming, setRevenueClaiming] = useState(false);
  const [claimFeeEstimate, setClaimFeeEstimate] =
    useState<ClaimFeeEstimate | null>(null);
  ...
  const handleRevenueClaim = useCallback(async () => {
    if (
      ...
      !revenueEligibility.distributionId ||
      !claimFeeEstimate
    )
      return;
    ...
    try {
      ...
      const gatekeeperAddress =
        process.env.NEXT_PUBLIC_GATEKEEPER_ADDRESS ||
        "8w1dD5Von2GBTa9cVASeC2A9F3gRrCqHA7QPds5pfXsM";
      ...
      console.log(
        "   Token Account Creation (USD):",
        claimFeeEstimate.accountCreationFeeUsd,
      );
      ...
      if (lamportsToSend <= 0 || !isFinite(lamportsToSend)) {
        throw new Error(
          `Invalid fee amount calculated: ${lamportsToSend} lamports`,
        );
      }
      ...
      if (
        transferInstruction &&
        transferInstruction.programId.toBase58() ===
      ...
      console.log(
        "   📤 Submitting claim to backend with transaction signature...",
      );
        ...
        if (user) {
          try {
            const { logTransaction } =
              await import("@/services/transactionHistoryService");
      ...
      if (user) {
        try {
          ...
          const { errorCode, errorMessage, technicalError } =
  ...
  const level = Math.min(
    50,
    Math.floor((userRewards?.weeklyRate || 0) / 10) + 1,
  );
      ...
      <main className="w-full max-w-[400px] md:max-w-[600px] lg:max-w-[800px] xl:max-w-[1000px] flex flex-col gap-5">
        ...
        <section className="flex flex-col gap-3">
          ...
          <StatsCard
            label="Mining Rate"
            value={
              rewardsLoading ? "..." : Math.round(userRewards?.weeklyRate || 0)
            }
            unit="MKIN/week"
            ...
            <div className="p-6">
              ...
              <div className="mb-6 bg-black/40 rounded-xl p-4 border border-[#27272a]">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="text-xs font-semibold text-[#fbbf24] bg-[#fbbf24]/10 px-2 py-0.5 rounded">
                    MKIN TOKEN
                  </span>
                </div>
                <div className="text-2xl font-bold text-white">
                  {(userRewards?.totalRealmkin || 0).toLocaleString(undefined, {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
                  })}{" "}
                  <span className="text-sm text-gray-400">MKIN</span>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      setWithdrawAmount(String(userRewards?.totalRealmkin || 0))
                    }
          ...
          >
            ...
            <div className="p-6 border-b border-white/10 flex justify-between items-center">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-purple-500/10 flex items-center justify-center border border-purple-500/30">
                  <span
                    className="material-symbols-outlined text-[#8b5cf6]"
                    style={{
                      fontVariationSettings:
                        "'FILL' 0, 'wght' 400, 'GRAD' 0, 'opsz' 24",
                    }}
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">
                    Send MKIN to any Realmkin address
                  </p>
                </div>
              </div>
              <button
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center ml-1">
                  ...
                  <span className="text-[10px] text-gray-500 font-mono">
                    Available:{" "}
                    {(userRewards?.totalRealmkin || 0).toLocaleString()} MKIN
                  </span>
                </div>
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-gray-500 group-focus-within:text-[#fbbf24] transition-colors">
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      setTransferAmount(String(userRewards?.totalRealmkin || 0))
                    }
            ...
            <div className="p-6 pt-2">
              ...
              <p className="text-center text-[10px] text-gray-500 mt-4 px-4 leading-relaxed">
                By confirming this transaction, you agree to the Realmkin
                Protocol terms. Please double check the recipient address before
                sending.
              </p>
            </div>
          </div>
